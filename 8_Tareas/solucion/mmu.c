...
paddr_t mmu_unmap_page(uint32_t cr3, vaddr_t virt) {
	...
	if ((pde_ptr->attrs & MMU_P) == 0){ //probablemente no entre nunca a este IF.
		tlbflush();
...
void copy_page(paddr_t dst_addr, paddr_t src_addr) {
	...
	uint8_t* dst = (uint8_t*)DST_VIRT_PAGE;		// no sabemos por que no podemos usar 'uint8_t* dst = DST_VIRT_PAGE;'
	uint8_t* src = (uint8_t*)SRC_VIRT_PAGE;
	for (size_t i = 0; i < PAGE_SIZE; i++) {
		// accedemos a SRC_VIRT_PAGE + i y DST_VIRT_PAGE + i y copiamos byte a byte

		dst[i] = src[i];		//se puede iterar page_size/4 veces (1024) y utilizar uint32_t para que esto sea mas eficiente?
	}
